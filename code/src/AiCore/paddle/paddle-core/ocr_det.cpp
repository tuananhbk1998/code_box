...
void DBDetector::Run(cv::Mat &img,
  ...
  int w = img.cols;
  int h = img.rows;
  ratio_h = float(96) / float(h);
  ratio_w = float(128) / float(w);

  cv::Mat srcimg;
  cv::Mat resize_img;
  img.copyTo(srcimg);
  //  std::cout<<"=============================max side: "<<this->max_side_len_ <<std::endl;
  // this->resize_op_.Run(img, resize_img, this->max_side_len_, ratio_h, ratio_w,
  //                      this->use_tensorrt_);
  cv::resize(img, resize_img, cv::Size(128, 96));
  // cv::Mat img_fp;
  // resize_img.convertTo(img_fp, CV_32FC3, 1.0 / 255.f);
  // resize_img = img_fp;

  // std::cout<<"=============================IMG row: "<<img.rows <<std::endl;
  // std::cout<<"=============================IMG cols: "<<img.cols <<std::endl;                     
  // std::cout<<"=============================Resize IMG row: "<<resize_img.rows <<std::endl;
  // std::cout<<"=============================Resize IMG cols: "<<resize_img.cols <<std::endl;
  ...
  cv::imwrite("./processing/pred_map.png",pred_map);
  cv::imwrite("./processing/cbuf_map.png",cbuf_map);
  cv::imwrite("./processing/bit_map.png",bit_map);
  cv::Mat dilation_map;
  cv::Mat dila_ele = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 2));
  cv::dilate(bit_map, dilation_map, dila_ele);
  cv::imwrite("./processing/dilation_map.png",dilation_map);
  // cv::waitKey(0);
