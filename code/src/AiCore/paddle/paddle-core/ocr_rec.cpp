...
std::string CRNNRecognizer::Run(std::vector<std::vector<std::vector<int>>> boxes,
  ...
  for (int i = (boxes.size() -1); i >= 0 ; i--) {
    ...
    // std::cout << str_res[0] << std::endl;
    // std::cout<< "\n  "  << std::endl; 
    
    // cv::Ptr<cv::freetype::FreeType2> ft2 = cv::freetype::createFreeType2();
    // ft2->loadFontData( "../font/NotoSansCJKjp-Regular.otf", 0 );
    // ft2->putText( img, "ABCDEFG", cv::Point( 10, 50 ), 30, cv::Scalar( 255, 255, 255 ), -1, cv::LINE_AA, false );
