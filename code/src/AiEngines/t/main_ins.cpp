...
int main()
{
	...
	settings.arListObjects = std::vector<std::string>{"xemay"};
    settings.cTimeOut = 5;
	...
	while(1)
	{
        ...
        for(auto out : outData)
		{
			...
			cv::putText(frame, "INSIDED", cv::Point(out.rect.x, out.rect.y), FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 1);
		}
