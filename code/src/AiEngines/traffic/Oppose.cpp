...
int Oppose::OP_Init()
{
    ...
    {
        // std::cerr << OPPOSE << " Initialize Oppose object failed." << std::endl;
        return -1;
    }
...
/*-------------------------------------------------------------------------------
Function: OP_Set(stSettingOppose_t stSetting)
...
/*-------------------------------------------------------------------------------
Function: OP_Exe(cv::Mat& mImage, std::vector<stOutputOppose_t>& stOpposes)
...
int Oppose::OP_Exe(cv::Mat& mImage, std::vector<stOutputOppose_t>& stOpposes)
{
    ...
    else
    {
        ...
        {
            // std::cerr << OPPOSE << " Execute Detector failed." << std::endl;
            return -1;
        }
            ...
            if (stTrack.isOutOfFrame == false)
            {
                if (m_strVehicle.size() == 0)
                {
                    ...
                    if (p != m_stTracks.end()) // if find track in list tracked
                    {
                        ...
                        cv::putText(mImage, std::to_string(m_stTracks[iIndex].iTrackID), m_stTracks[iIndex].riBox.tl(), FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0,0,0));
                        cv::rectangle(mImage, m_stTracks[iIndex].riBox, cv::Scalar(0,0,255));
                        if (OP_bCheckLane(stTrack, m_ptRegion))
                        {
                            if (m_stTracks[iIndex].iCountCheck <= MAX_COUNT_CHECK)
                            {
                                ...
                                if (bLess)
                                {
                                    ...
                                    {
                                        m_stTracks[iIndex].iCountCheck -= 1;
                                        if (m_stTracks[iIndex].iCountCheck < 0) m_stTracks[iIndex].iCountCheck = 0;
                ...
                else 
                {
                    if (find(m_strVehicle.begin(), m_strVehicle.end(), stTrack.m_type) != m_strVehicle.end())
                    {
                        ...
                        if (p != m_stTracks.end()) // if find track in list tracked
                        {
                            ...
                            std::string text = std::to_string(m_stTracks[iIndex].iTrackID) + " - " + std::to_string(m_stTracks[iIndex].iCountCheck);
                            cv::putText(mImage, text, m_stTracks[iIndex].riBox.tl(), FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0,0,0));  
                        }
                        else // if find new track
                        {
                            if (OP_bCheckLane(stTrack, m_ptRegion))
...
/*-------------------------------------------------------------------------------
Function: OP_bCheckLane(TrackingTrace stTrack, std::vector<cv::Point> ptArea)
Purpose: 
...
/*-------------------------------------------------------------------------------
Function: OP_fCalDistanceTrace(TrackingTrace stTrack)
Purpose: 
Parameters: stTrack
Return: 
Comments: 
